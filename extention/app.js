chrome.history.search({ text: '' }, results => {
    ...
    setTimeout(() => {
        ...
        document.querySelector("#twitterShareButton").addEventListener("click", () => chrome.tabs.create({ url: `https://twitter.com/intent/tweet?text=${$tweetText.replace(" ", "%20").replace("#", "%23")}@ryncmrfrd%20%23tweetmyhistory` }) );
